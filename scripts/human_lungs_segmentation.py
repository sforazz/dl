...
def data_split(data_dir, train_len=0):
    ...
    for root, _, files in os.walk(data_dir): 
        for name in files: 
            if name.endswith('.nii.gz') and 'lungs' not in name and 'vol' in name: 
                data.append(os.path.join(root, name))
            elif name.endswith('.nii.gz') and 'lungs' in name and 'vol' in name: 
...
data_dir = '/mnt/sdb/human_lungs_data/thorax/'
len_train = 1400  #102668
...
for i in range(5):
    ...
    try:
        train_files = np.load('/mnt/sdb/human_lungs_data/thorax/train_files.npy')
        validation_files = np.load('/mnt/sdb/human_lungs_data/thorax/validation_files.npy')
        train_masks = np.load('/mnt/sdb/human_lungs_data/thorax/train_masks.npy')
        validation_masks = np.load('/mnt/sdb/human_lungs_data/thorax/validation_masks.npy')
        ...
        np.save('/mnt/sdb/human_lungs_data/thorax/train_files.npy', train_files)
        np.save('/mnt/sdb/human_lungs_data/thorax/validation_files.npy', validation_files)
        np.save('/mnt/sdb/human_lungs_data/thorax/train_masks.npy', train_masks)
        np.save('/mnt/sdb/human_lungs_data/thorax/validation_masks.npy', validation_masks)
    if i == 0:
