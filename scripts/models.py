...
from keras.layers.convolutional import Conv2D, Deconv2D, ZeroPadding2D, UpSampling2D, UpSampling3D, Conv3D
...
def up_conv_block_unet(x, x2, f, name, bn_mode, bn_axis, bn=True, dropout=True):
...
def DCGAN_2D_sobel(generator, discriminator_model, img_dim, patch_size, image_dim_ordering):
    
    gen_input = Input(shape=img_dim, name="DCGAN_input")

    generated_image = generator(gen_input)

    generated_edge = Lambda(lambda inputs: sobel_edges_2D(inputs))(generated_image)
...
def generator_unet_3D_upsampling(img_dim, bn_mode, model_name="generator_unet_3D_upsampling"):
    ...
    list_decoder = [up_conv_block_unet_3D(list_encoder[-1], list_encoder[-2],
                                       list_nb_filters[0], "unet_upconv3D_1", bn_mode, bn_axis, dropout=True)]
    for i, f in enumerate(list_nb_filters[1:]):
        name = "unet_upconv3D_%s" % (i + 2)
        # Dropout only on first few layers
        if i < 2:
            d = True
        else:
            d = False
        conv = up_conv_block_unet_3D(list_decoder[-1], list_encoder[-(i + 3)], f, name, bn_mode, bn_axis, dropout=d)
        list_decoder.append(conv)
...
def DCGAN_discriminator_3D(img_dim, nb_patch, bn_mode, model_name="DCGAN_discriminator_3D", use_mbd=True):
    """
    Discriminator model of the DCGAN
    args : img_dim (tuple of int) num_chan, height, width
           pretr_weights_file (str) file holding pre trained weights
...
def DCGAN_3D(generator, discriminator_model, img_dim, patch_size, image_dim_ordering):
...
def DCGAN_3D_no_sobel(generator, discriminator_model, img_dim, patch_size, image_dim_ordering):
...
#         model = build_unet_generator()
    ...
    if model_name == "generator_unet_3D_upsampling":
        model = generator_unet_3D_upsampling(img_dim, bn_mode, model_name=model_name)
#         model = build_unet_generator()
        model.summary()
        if do_plot:
            from keras.utils import plot_model
    ...
    if model_name == "DCGAN_discriminator_3D":
        model = DCGAN_discriminator_3D(img_dim, nb_patch, bn_mode, model_name=model_name, use_mbd=use_mbd)
        model.summary()
        if do_plot:
            from keras.utils import plot_model
            plot_model(model, to_file="../../figures/%s.png" % model_name, show_shapes=True, show_layer_names=True)
...
if __name__ == "__main__":
    ...
    load("generator_unet_upsampling", (256, 256, 1), 16, 2, False, 32)
